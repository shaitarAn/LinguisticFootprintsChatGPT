...
for group_name, group_features in feature_groups.items():
    ...
    for dir_name in os.listdir(f"../results/per_feature"):
        ...
        for corpus in os.listdir(f"../results/per_feature/{feature_name}"):

            file_path = os.path.join(f"../results/per_feature/{feature_name}", corpus)

            corpus = corpus.split('.')[0]

            df = pd.read_csv(file_path)

            if not os.path.exists(f"../visualizations/boxplots/feat_groups/{group_name}/{corpus}"):
                os.makedirs(f"../visualizations/boxplots/feat_groups/{group_name}/{corpus}")
            ...
            plt.savefig(f"../visualizations/boxplots/feat_groups/{group_name}/{corpus}/{feature_name}.pdf", bbox_inches='tight')
