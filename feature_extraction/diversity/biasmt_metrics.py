...
def get_lemmas(sentences, nlpD, system_name, freq_voc=None):
    ...
    if os.path.exists(root + system_name + ".spacy_udpipe.lemmas"):
        logging.debug("Lemmas dict loading from file")
        with open(root + system_name + ".spacy_udpipe.lemmas", "rb") as SpUpM:
        ...
        with open(root + system_name + ".spacy_udpipe.lemmas", "wb") as PoF:
    ...
    with open(root + system_name + ".lemmas", "w") as oF:
...
def compute_gram_diversity(sentences, lang="en", system_name="", freq_voc=None):
    ''' Computing metric
        ...
        :returns: a socre (float)
    '''
    nlpD = spacy_udpipe.load("de").tokenizer
