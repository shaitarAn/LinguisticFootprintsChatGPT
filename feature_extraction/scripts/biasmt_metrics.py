...
def get_lemmas(sentences, nlpD, system_name, corpus, freq_voc=None):
    ...
    if os.path.exists(root + corpus + "/" + system_name + ".spacy_udpipe.lemmas"):
        logging.debug("Lemmas dict loading from file")
        with open(root + corpus + "/" + system_name + ".spacy_udpipe.lemmas", "rb") as SpUpM:
        ...
        for doc in nlps:
            for token in doc:
                ...
                if lemma in lemmas:  # existing lemma
                    ...
                    # if this is the first time we have a lemma then there are no tokens
                    lemmas[lemma] = {}
                    lemmas[lemma][tokenLow] = 1
        if not os.path.exists(root + corpus + "/"):
                os.makedirs(root + corpus + "/")
        with open(root + corpus + "/" + system_name + ".spacy_udpipe.lemmas", "wb") as PoF:
            pickle.dump(lemmas, PoF)

        logging.debug("Lemmas dict built and saved")

    print("Length of all lemmas: " + str(len(lemmas)))
    singleton_lemmas = [lemma + "\t" + str(len(lemmas[lemma]))
                        for lemma in lemmas if len(lemmas[lemma]) < 2]
    print("Length of singleton lemmas: " + str(len(singleton_lemmas)))
    singleton_matching_lemmas = []

    with open(root + corpus + "/" + system_name + ".lemmas", "w") as oF:
        oF.write("\n".join([lemma + ": " + "\t".join(str(f) + "|" + str(g) for (f, g)
                                                     in zip(lemmas[lemma].keys(), lemmas[lemma].values())) for lemma in lemmas]))
    ...
    # print(sorted(singleton_matching_lemmas))
...
def shannon_diversity(wordFormDict):
    ...
    print(wordFormDict.values())
    print("sum of wordFormDict.values()", N)
...
def compute_shannonDiv_perLemma(nestedDict):
    ''' Computes the shannon diversity for every lemma
        example input : {lemma1:{wordf1: count1, wordf2: count2}, lemma2 {wordform1: count1}}
        output {lemma1: simpDiv1, lemma2:simpDiv2}
...
def compute_ld_metric(metric_func, sentences, sample_idxs, iters):
    ...
    scores = Parallel(n_jobs=-1)(delayed(eval(metric_func))
                                 ([sentences[j] for j in sample_idxs[i]]) for i in range(iters))
